...
- hosted registry and mirror service validation in `styio-cloud`
...
`pafio` remains the offline-capable local package manager. This runbook
...
Build the VM bundle from a clean `styio-cloud` checkout:
...
- `/opt/styio-cloud-registry` runtime scripts and Python modules
- `/var/lib/styio-cloud/registry/v2` initialized registry static root
- `/var/lib/styio-cloud/registry/v2-keys` server-side registry role keys
- `/etc/styio-cloud/styio-registry.env` deployment environment file
...
```text
sudo ./install.sh --pafio-bin /opt/pafio/bin/pafio --control-bind 127.0.0.1 --read-bind 127.0.0.1
```
...
```text
/opt/styio-cloud-registry/scripts/registry-v2-vm-smoke.py \
...
```text
./scripts/registry-server-gate.py --registry-root https://registry.example.invalid --pafio-bin ./build-codex/bin/pafio --json
```
...
```text
./scripts/registry-server-gate.py --registry-root https://registry-upload.example.invalid --publish-header 'X-Pafio-Write-Token: <write-token>' --pafio-bin ./build-codex/bin/pafio --json
```
...
```text
./scripts/registry-server-gate.py --registry-root https://registry-upload.example.invalid --publish-policy-file /etc/pafio/publish-policy.toml --pafio-bin ./build-codex/bin/pafio --json
```

If the deployment links a private security module and already provisions a named profile under `PAFIO_HOME/server/registry/publish-profiles/`, validate that path directly:

```text
pafio publish --manifest-path path/to/pafio.toml --registry https://registry-upload.example.invalid --registry-profile write-dev
```
...
```text
./scripts/registry-server-gate.py --publish-root https://registry-upload.example.invalid --fetch-root https://registry.example.invalid --sync-timeout-seconds 30 --pafio-bin ./build-codex/bin/pafio --json
```
