...
Every `styio-cloud` workspace defaults to compiling Styio code. The same
...
`styio-cloud` owns the hosted workspace and cloud execution side of this
model. `styio` owns language and compiler semantics. `pafio` owns local
