...
json BuildPackagePayload(const pafio::ResolvedPackage &package)
{
  json payload = {
      {"id", package.id},
...
json BuildTargetPayload(const pafio::ResolvedPackage &package)
{
  json payload = {
  ...
  {
    payload["lib"] = pafio::CanonicalAbsolutePath(package.root_dir / package.package.lib->path).string();
  }
  for (const auto &bin : package.package.bins)
  {
    payload["bins"].push_back({
        {"name", bin.name},
        {"path", pafio::CanonicalAbsolutePath(package.root_dir / bin.path).string()},
    });
  }
  for (const auto &test : package.package.tests)
  {
    payload["tests"].push_back({
        {"name", test.name},
        {"path", pafio::CanonicalAbsolutePath(package.root_dir / test.path).string()},
    });
  }
  return payload;
}

json BuildDependencyEdgesPayload(const pafio::ResolvedGraphResult &graph)
{
...
json BuildPackageDistributionPayload(const pafio::ResolvedGraphResult &graph)
{
  int path_count = 0;
  int git_count = 0;
    ...
    const fs::path &pafio_home)
{
  json payload = {
      ...
      {"source_origin", pafio::ResolveSourceOriginBaseline()},
      {"source_state_root", pafio::SourceStateRoot(pafio_home).string()},
  };
  if (state.source_revision.has_value())
  {
    payload["source_revision"] = *state.source_revision;
  }
  if (const char *env_source_root = std::getenv("PAFIO_STYIO_SOURCE_ROOT"); env_source_root != nullptr &&
                                                             env_source_root[0] != '\0')
  {
    payload["explicit_source_root"] = pafio::CanonicalAbsolutePath(env_source_root).string();
  }
  payload["project_state_root"] = pafio::ProjectStateRootForManifest(manifest_path).string();
  return payload;
}
    ...
    const ResolveOptions &resolve_options)
{
  ...
  return {
      ...
      {"source_state", BuildSourceStatePayload(manifest_path, toolchain_state, tool_status.pafio_home)},
