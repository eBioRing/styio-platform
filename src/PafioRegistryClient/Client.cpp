...
std::pair<std::string, std::string> SplitPackageName(const std::string &package_name)
{
  ...
  {
    throw pafio::FetchError("registry package name must match namespace/name: " + package_name);
  }
  return {
...
fs::path NormalizeRelativeRegistryPath(const std::string &relative_path, const std::string &context)
{
  ...
  {
    throw pafio::FetchError(context + " must be a non-empty relative path");
  }
  const fs::path path(relative_path);
  if (path.is_absolute())
  {
    throw pafio::FetchError(context + " must be relative: " + relative_path);
  }
  const fs::path normalized = path.lexically_normal();
  const std::string text = normalized.generic_string();
  if (text.empty() || text == "." || text == ".." || text.starts_with("../"))
  {
    throw pafio::FetchError(context + " escapes the registry root: " + relative_path);
  }
  return normalized;
}
