...
{
  const pafio::ResolvedPackage *package = nullptr;
  std::string kind;
  std::string name;
  fs::path file;
};
...
void ValidateIntent(std::string_view intent)
{
  ...
  {
    throw pafio::PlanError("unsupported compile-plan intent for the current native core: " + std::string(intent));
  }
}
...
std::string ReadFile(const fs::path &path)
{
  ...
  if (!in)
  {
    throw pafio::PlanError("failed to read file required for compile-plan: " + path.string());
  }
...
fs::path AbsoluteTargetPath(const pafio::ResolvedPackage &package, const std::string &relative_path)
{
...
void RequireExistingSourceFile(const fs::path &path, std::string_view context)
{
  if (!fs::exists(path))
  {
    throw pafio::PlanError(std::string(context) + " source file not found: " + path.string());
  }
  if (!fs::is_regular_file(path))
  {
    throw pafio::PlanError(std::string(context) + " source path is not a file: " + path.string());
  }
}

const pafio::ResolvedPackage &RequirePackageById(
    const std::unordered_map<std::string, const pafio::ResolvedPackage *> &packages_by_id,
    const std::string &package_id)
{
  const auto found = packages_by_id.find(package_id);
  if (found == packages_by_id.end())
  {
    throw pafio::PlanError("resolver graph references unknown package id: " + package_id);
  }
  return *found->second;
}

std::vector<const pafio::ResolvedPackage *> CollectRootPackages(const pafio::ResolvedGraphResult &graph)
{
  std::unordered_map<std::string, const pafio::ResolvedPackage *> packages_by_id;
  for (const pafio::ResolvedPackage &package : graph.packages)
  {
    packages_by_id.emplace(package.id, &package);
  }

  std::vector<const pafio::ResolvedPackage *> roots;
...
const pafio::ResolvedPackage &ResolveEntryPackage(const pafio::BuildPlanRequest &request, const pafio::ResolvedGraphResult &graph)
{
  const std::vector<const pafio::ResolvedPackage *> roots = CollectRootPackages(graph);
  if (roots.empty())
  {
    throw pafio::PlanError("compile-plan generation requires at least one root package");
  }

  if (request.package_name.has_value())
  {
    std::vector<const pafio::ResolvedPackage *> matches;
    for (const pafio::ResolvedPackage *candidate : roots)
    {
    ...
    if (matches.empty())
    {
      throw pafio::PlanError("selected package is not a root package of the active manifest: " + *request.package_name);
    }
  ...
  throw pafio::PlanError("workspace build is ambiguous; select a root package with --package <namespace/name>");
}

SelectedTarget ResolveEntryTarget(const pafio::BuildPlanRequest &request, const pafio::ResolvedPackage &package)
{
  ...
  {
    throw pafio::PlanError(request.intent + " target selection cannot combine --lib and --bin");
  }
  if (request.select_lib && request.test_name.has_value())
  {
    throw pafio::PlanError(request.intent + " target selection cannot combine --lib and --test");
  }
  if (request.bin_name.has_value() && request.test_name.has_value())
  {
    throw pafio::PlanError(request.intent + " target selection cannot combine --bin and --test");
  }
  ...
  if (request.select_lib)
  {
    if (require_binary)
    {
      throw pafio::PlanError("run requires a binary target and does not accept --lib");
    }
    if (require_test)
    {
      throw pafio::PlanError("test requires an explicit [[test]] target and does not accept --lib");
    }
    if (!package.package.lib.has_value())
    {
      throw pafio::PlanError("selected package does not define [lib]: " + package.package.name);
    }
  ...
  {
    for (const pafio::BinTarget &bin : package.package.bins)
    {
    ...
    throw pafio::PlanError("selected package does not define [[bin]].name = \"" + *request.bin_name + "\"");
  }

  if (request.test_name.has_value())
  {
    for (const pafio::TestTarget &test : package.package.tests)
    {
    ...
    throw pafio::PlanError("selected package does not define [[test]].name = \"" + *request.test_name + "\"");
  }
  ...
  if (require_test)
  {
    if (test_count == 1U)
    {
      const pafio::TestTarget &test = package.package.tests.front();
      const fs::path file = AbsoluteTargetPath(package, test.path);
      RequireExistingSourceFile(file, "test");
      return {
          .package = &package,
    ...
    if (test_count == 0U)
    {
      throw pafio::PlanError("selected package does not define any [[test]] target: " + package.package.name);
    }
    throw pafio::PlanError("test target is ambiguous for package '" + package.package.name + "'; select --test <name>");
  }
  if (require_binary)
  {
    if (bin_count == 1U)
    {
      const pafio::BinTarget &bin = package.package.bins.front();
      const fs::path file = AbsoluteTargetPath(package, bin.path);
      RequireExistingSourceFile(file, "binary");
      return {
          .package = &package,
    ...
    if (bin_count == 0U)
    {
      throw pafio::PlanError("selected package does not define any [[bin]] target: " + package.package.name);
    }
    throw pafio::PlanError("run target is ambiguous for package '" + package.package.name + "'; select --bin <name>");
  }
...
ProfileConfig ResolveProfile(std::string_view profile_name)
{
  ...
  throw pafio::PlanError("unsupported build profile: " + std::string(profile_name));
}

void ValidateUniformToolchain(const pafio::ResolvedGraphResult &graph)
{
  if (graph.packages.empty())
  {
    throw pafio::PlanError("compile-plan generation requires at least one resolved package");
  }

  const pafio::ResolvedPackage &baseline = graph.packages.front();
  for (const pafio::ResolvedPackage &package : graph.packages)
  {
...
std::string BuildStdPackageId(const pafio::ResolvedPackage &entry_package)
{
...
std::vector<std::string> BuildPackageOrder(const pafio::ResolvedGraphResult &graph)
{
  std::unordered_map<std::string, const pafio::ResolvedPackage *> packages_by_id;
  for (const pafio::ResolvedPackage &package : graph.packages)
  {
  ...
  std::function<void(const std::string &)> visit = [&](const std::string &package_id) {
    ...
    if (state == VisitState::kActive)
    {
      ...
      throw pafio::PlanError("compile-plan requires an acyclic package graph: " + Join(cycle, " -> "));
    }

    visit_state[package_id] = VisitState::kActive;
    stack.push_back(package_id);

    const pafio::ResolvedPackage &package = RequirePackageById(packages_by_id, package_id);
...
std::string BuildSourceHash(const pafio::ResolvedGraphResult &graph)
{
...
json BuildTargetsJson(const pafio::ResolvedPackage &package)
{
  json bins = json::array();
  std::vector<pafio::BinTarget> ordered_bins = package.package.bins;
  std::sort(
      ordered_bins.begin(),
      ordered_bins.end(),
      [](const pafio::BinTarget &left, const pafio::BinTarget &right) {
        return left.name < right.name;
      });
  for (const pafio::BinTarget &bin : ordered_bins)
  {
  ...
  std::vector<pafio::TestTarget> ordered_tests = package.package.tests;
  std::sort(
      ordered_tests.begin(),
      ordered_tests.end(),
      [](const pafio::TestTarget &left, const pafio::TestTarget &right) {
        return left.name < right.name;
      });
  for (const pafio::TestTarget &test : ordered_tests)
  {
...
json BuildPackageJson(const pafio::ResolvedPackage &package)
{
  json dependency_aliases = json::object();
  for (const pafio::ResolvedDependencyAlias &alias : package.dependency_aliases)
  {
...
BuildPlanResult WriteBuildCompilePlan(const BuildPlanRequest &request)
{
  ...
  const fs::path build_root = workspace_root / ".pafio" / "build" / cache_key;
